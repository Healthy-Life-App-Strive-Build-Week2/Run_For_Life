...
y_train = train_users['target'].map({"Bus":3, "Car":2, "Still": 3, "Train":1, "Walking":3 })
y_val = test_users['target'].map({"Bus":3, "Car":2, "Still": 3, "Train":1, "Walking": 3})
...
for model_name, model in tree_pipes.items():
    ...
    pred_vs = pd.DataFrame({"Predictions": pred,
                            "Actuals": y_val})
    pred_vs.to_csv(model_name+"_4cl_vs.csv")
